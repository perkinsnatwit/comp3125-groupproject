...
df["application_volume"] = (
    df["application_volume"].astype(str).str.replace(",", "").astype(int)
)
...
sns.barplot(x="admission_rate", y="colleges", data=df, color="skyblue")
...
sns.scatterplot(x="admission_rate", y="graduate_rate_6yr", hue="colleges", data=df, s=100)
...
plt.figure(figsize=(8, 6))
sns.scatterplot(x="application_volume", y="admission_rate", hue="colleges", data=df, s=120)
...
plt.figure(figsize=(10, 6))
df_melt = df.melt(id_vars="colleges", value_vars=["graduate_rate_4yr", "graduate_rate_6yr"], var_name="Graduation Type", value_name="Rate")
